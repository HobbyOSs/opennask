...
CMakeLists.txt 内には `od` という依存関係グループが定義されています。これは主に以下の目的で使用されます。

- **gosk ビルドと wine nask ビルドの比較**: 同じアセンブリソース (`.nas`) から gosk (プロジェクト固有のアセンブラ) と wine nask (Windows版 NASK を Wine 経由で実行) でそれぞれバイナリファイル (`.bin`) を生成します。
- **hexdump と diff**: 生成された2つのバイナリファイルを `hexdump -C` でテキスト形式に変換し、`diff` コマンドで比較します。
- **差異の検出**: これにより、gosk と wine nask の間でアセンブル結果に差異がないかを確認できます。差異がある場合、どちらかのアセンブラの実装に問題があるか、あるいは意図しない挙動をしている可能性があります。

### 実行方法

- **プロジェクト全体の `od` ターゲットを実行**:
  ```bash
  cd build
  ninja od
  ```
  これにより、`add_dependencies(od ...)` で指定されたすべての比較ターゲットが実行されます。

- **特定の日付・プロジェクトの `od` ターゲットを実行**:
  CMakeLists.txt 内で定義されている特定の比較ターゲット名を指定して実行します。ターゲット名は通常 `<day>_<project>_<file>_od` の形式になっています。
  例えば、`03_day/harib00i` の `asmhead.nas` の比較のみを実行したい場合は、以下のコマンドを実行します。
  ```bash
  cd build
  ninja 03_day_harib00i_asmhead_od
  ```
  同様に、`ipl10.nas` の比較を実行する場合は以下のようにします。
  ```bash
  cd build
  ninja 03_day_harib00i_ipl10_od
  ```
