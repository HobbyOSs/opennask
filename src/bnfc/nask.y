...
;
Statement : T_Label { $$ = new LabelStmt($1); result->statement_ = $$; }
...
Exp : Exp _PLUS Exp { $$ = new PlusExp($1, $3); result->exp_ = $$; }
  ...
  | Exp _PERCENT Exp { $$ = new ModExp($1, $3); result->exp_ = $$; }
...
;
DataType : _KW_BYTE { $$ = new ByteDataType(); result->datatype_ = $$; }
  | _KW_WORD { $$ = new WordDataType(); result->datatype_ = $$; }
  | _KW_DWORD { $$ = new DwordDataType(); result->datatype_ = $$; }
;
